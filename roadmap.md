...
Topo du 15 août 2026. Chaque piste : ce que c'est, pourquoi ça compte, ce que ça coûte.
La numérotation est un ordre de lecture, **pas** un ordre de priorité — c'est à trancher.
...
### 1. **Banc de vitesse WASM — le test qui décide de tout le reste**
...
**Verdict attendu : sous ×3 de gain brut, la piste 3 est morte** et on arrête d'y penser.
Coût : 2-3 jours. À faire **avant** toute décision. Priorité :

### 2. **Autoriser le WASM dans la webview (une ligne)**
...
Une ligne dans [webview-html.ts:74](src/webview-html.ts#L74). À faire au **jour 1** de la piste 1, sinon le banc ne démarre même pas.
Coût : 10 minutes. Priorité :

### 3. **Réécrire le cœur du Pico en WASM**
...
C'est cinq semaines de travail sur un morceau très délicat (les interruptions et les exceptions du processeur), et **la piste 1 peut la tuer en 2 jours**. À ne lancer qu'après le banc, et seulement si le banc est bon.
Coût : 25-38 jours. Bloqué par la piste 1. Priorité :

### 4. **Regarder `cts2c` : le cœur WASM peut-être déjà écrit par quelqu'un d'autre**
Une équipe extérieure a écrit un outil qui **traduit automatiquement** le code de l'émulateur (TypeScript) en langage C, annoncé 2 à 4 fois plus rapide — exactement le gain visé par la piste 3. Leur but à eux est un programme classique, mais un fichier C se compile aussi en WASM.
Si ça marche, c'est **cinq semaines de travail remplacées par quelques jours**. À sonder juste après le banc.
Coût : sondage 2-3 jours. Priorité :

---

## Nouvelles cartes : Pico 2 et Pico 2 W

Le Pico 2 n'est pas un Pico plus rapide : c'est un **autre processeur** (Cortex-M33, ou RISC-V au choix). La bibliothèque qu'utilise Kablix n'en simule rien, et le MicroPython du Pico 2 ne tournera jamais sur notre cœur actuel. Écrire ça de zéro serait plus gros que la piste 3. **Mais quelqu'un l'a déjà écrit.**

### 5. **Évaluer `rp2350js`, la bibliothèque qui simule déjà le Pico 2**
...
Coût : 2-3 jours. Priorité :

### 6. **Vérifier leurs manques AVANT de s'emballer**
...
Si ces manques sont réels et profonds, la piste 5 s'arrête là, quelle que soit sa qualité par ailleurs. **C'est LA question du jour 1.**
Coût : compris dans la piste 5. Priorité :

### 7. **Intégrer le Pico 2 et le Pico 2 W dans Kablix**
Si les pistes 5 et 6 passent. Leur bibliothèque n'est pas remplaçable telle quelle (leur façon de l'appeler a changé) et n'est pas publiée sur npm : il faut la copier dans le projet et écrire une couche d'adaptation. Notre optimisation maison (+30 %) tombe : leurs 686 modifications ont les leurs, à re-mesurer.
Ensuite : dessiner les deux nouvelles cartes, ajouter leurs firmwares, leurs tests, leurs fiches d'aide.
Coût : 10-20 jours. Bloqué par les pistes 5 et 6. Priorité :
...
### 8. **Sonder l'émulateur officiel Espressif pour l'ESP32-C3**
...
### 9. **Ajouter la famille ESP32 à Kablix**
Si les crochets de la piste 8 existent. Ce n'est **pas** une variante du Pico : c'est un **troisième moteur** à côté de l'AVR et du RP2040, avec son brochage, ses dessins, son catalogue, ses tests et ses fiches d'aide.
Coût : 20-40 jours. Bloqué par la piste 8. Priorité :

---

## Confort de développement

### 10. **Faire tourner les tests en parallèle**
...
Coût : à chiffrer. Priorité :

### 11. **Alléger le bundle de la webview**
...
Coût : mesure d'abord. Priorité :

---

## Points déjà tranchés — ne pas rouvrir

1. **« Faire comme Wokwi » n'a pas d'objet.** La bibliothèque `rp2040js` qu'utilise Kablix **est** celle de Wokwi (même auteur, même dépôt), comme `avr8js`. Kablix fait tourner la leur, optimisée plus loin (+30 %). Un cœur WASM serait une **avance** sur eux, pas un rattrapage.
2. **Ne jamais dessiner un faux « Pico 2 ».** Mettre une carte Pico 2 dans la palette qui serait en réalité simulée par le cœur du Pico 1 : l'élève écrirait pour une carte et exécuterait sur une autre. Le mensonge se paie au premier programme qui ne tourne pas pareil sur le vrai matériel.
3. **ESP32 classique (Xtensa) : bloqué par la licence, pas par la technique.** Les seuls émulateurs sérieux sont des dérivés de QEMU, donc sous licence GPL — incompatible avec la distribution d'une extension MIT comme Kablix. Wokwi le fait, mais eux ne distribuent pas un paquet installable. À ne pas engager sans avis clair.
4. **ESP8266 : rien de sérieux à émuler.** Ne pas le promettre.
