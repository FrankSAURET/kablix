...
Grove DMX512 board (Seeed Studio): an **SP3485** line driver turning the UART of the board into a DMX512 output on a **3-pin XLR** socket. Library part: it is installed through the component manager.
...
Two grounds, hence two names: the drawing says “GND” on both sides, the netlist tells them apart with `GND.1` and `GND.2` — like the `Com.1` / `Com.2` of the relay.

## Wiring

- **SIG** to a transmit pin: `1` (TX) on Uno, `1` / `18` / `16` / `14` on Mega, `GP0` on Pico. With `DmxSimple`, any pin will do (3 by default).
- **VCC** to +5 V, **GND.1** to ground.
- On the XLR side, `+` / `−` / `GND.2` to the [fixture](spot.md), daisy-chained for the next ones.
