...
Schéma de la réponse :
{
  ...
  "pinRoles": { "AO": "SORTIE" }
    ...
    <ol class="steps">
      ...
      <li><strong>Librairies (modules <code>.py</code>)</strong> : vos modules perso sont rendus importables automatiquement au lancement — placez-les <strong>à côté du script</strong> (même dossier) ou dans un sous-dossier <code>lib/</code>. <code>import ma_lib</code> fonctionne alors sans configuration (les modules sont injectés dans <code>sys.modules</code> ; le Pico simulé n'a pas de système de fichiers).</li>
      <li><strong>Compiler &amp; exécuter</strong> : le fichier <code>.py</code> étant actif (cf. chip 📄), cliquez <strong>▶ Démarrer</strong>. Le firmware démarre puis le script (précédé des modules) est injecté via le raw REPL.</li>
