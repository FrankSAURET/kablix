...
- Chaque lot : todo.md (✅/⏳/⬜, numéro de version AU-DESSUS de ses items) + **CHANGELOG complété** + bump du `buildNumber` + build + commit + push.
- **CHANGELOG rempli au fil de l'eau** (règle globale, 12/09/2026) : les entrées du lot vont sous le numéro de la PROCHAINE publication, avec `prochaine publication` à la place de la date — ici `## 2026.9.4 (prochaine publication)`. « On prépare la publication » = ce numéro est gardé et la date du jour remplace la mention. Toujours en **français**, côté utilisateur, en trois parties (Nouveauté / Modification / Correction).
- **Une section datée du CHANGELOG = une version PUBLIÉE.** Ne jamais y ajouter d'entrée, ne jamais la rouvrir. Doute sur ce qui est en ligne → vérifier (historique git, Marketplace) ou demander à Frank.
