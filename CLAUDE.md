...
- Deux numéros : **publique** `ANNÉE.MOIS.incrément` (`version` de package.json, celui de la prochaine publication, bumpé à la publication seulement, incrément remis à 0 au changement de mois) et **interne dev** `buildNumber` (4e segment, démarre à 1, ne repart jamais à 0, bumpé à CHAQUE lot). Affichage : `src/version.ts` (`versionAffichee()` = 4 segments hors production, `versionPublique()` pour les fichiers utilisateur).
