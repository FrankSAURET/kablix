...
3. ⏳ Anneau neopixel ne marche toujours pas sur Pico/MicroPython — cause racine identifiée (v2026.7.82), confirmé spécifique à rp2040js et NON à Kablix : le bit-bang WS2812 fonctionne correctement sur AVR/avr8js (v2026.7.85, testé bout-en-bout). Sur Pico, seul recours actuel : utiliser le PIO (`rp2_pio_ws2812` officiel) plutôt que `machine.bitstream`/`neopixel.NeoPixel`, si le programme PIO échantillonne différemment — pas encore vérifié. Variables locales (r,g,b) jamais affichées en pas-à-pas : bug distinct, pas encore traité (`__kx_vars()` dans `pydebug.ts` ne lit que `globals()`, jamais les frames de fonction/méthode).
4. ⏳ neopixel ne marche pas non plus (la led unique) sur Pico/MicroPython — même cause racine que l'item 3 ; confirmé fonctionnel sur Arduino/AVR (v2026.7.85).
5. ⏳ Matrice neopixel ne marche pas non plus sur Pico/MicroPython — idem item 3 ; confirmé fonctionnel sur Arduino/AVR (v2026.7.85).
6. ⏳ TFT display (ILI9341, SPI) ne marche pas sur Pico/MicroPython — cause racine identifiée (v2026.7.84), confirmé spécifique à rp2040js et NON à Kablix : le SPI multi-octets fonctionne correctement sur AVR/avr8js (v2026.7.85, testé bout-en-bout, CASET/RASET/RAMWR complets).
