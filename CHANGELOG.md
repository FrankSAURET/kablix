...
- **L'analyseur logique montre de nouveau ses courbes à la réouverture d'un projet.** Trois défauts se cumulaient : la capture enregistrée était effacée juste après avoir été affichée — l'onglet annonçait « aucune sonde » par-dessus une mesure entière —, les courbes retrouvées étaient rangées sous les mauvaises pinces quand l'ordre des sondes avait changé depuis l'enregistrement, et l'ouverture d'un **second projet** laissait en place les pistes du précédent : la nouvelle capture n'avait nulle part où se dessiner. Chaque courbe suit maintenant la broche sur laquelle elle a été mesurée, quel que soit le projet ouvert avant.
