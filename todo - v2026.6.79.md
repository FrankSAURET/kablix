...
1. ✅ Schéma interne du clavier dessiné (3×4 et 4×4) → intégré (cf. v2026.6.45).svg/pot-schema.edit.svgsrc/webview/diagram/internal-wiring.mtsscripts/_gen-pot-schema.mjs
...
2. ✅ **Fermer l'afficheur série + icône écran (barre de simulation)**. Croix `✕` (`#close-serial`) dans l'en-tête du panneau série + bouton écran 🖥 (`#toggle-serial`) tout à droite de la barre de simulation gauche → ouvrent/ferment la section (`setSerialVisible`, `serialVisible` persisté dans l'uiState, bouton actif = classe `primary`).
3. ✅ **Bouton fichier jaune sur rouge si aucun fichier**. `#code-file` reçoit la classe `--nofile` (jaune sur rouge, [`styles.css`](media/styles.css)) tant qu'aucun fichier de code n'est associé ; au lancement de la simulation sans fichier, il **clignote 3 fois** (`--blink`, keyframes `kablix-nofile-blink`, retiré à `animationend`). Repasse au style normal dès qu'un fichier est choisi (`hasCodeFile`).
4. ✅ **Recentrage auto à l'ouverture d'un projet**. `loadProject` appelle `editor.fitView()` (différé d'une frame pour laisser les corps prendre leur taille), comme le bouton « recentrer et ajuster ».
5. ✅ **État propre au (re)lancement et à l'arrêt**. `stopRun` vide désormais la console (`clearSerial`) et réinitialise les composants (`editor.resetVisuals`) : à l'arrêt de la simulation **et** au (re)chargement d'un programme (Python inclus, car `startRun` commence par `stopRun`). Bouton « réinitialiser » simplifié (le reset est déjà dans `stopRun`).
src/webview/sim.mtssrc/panel.tsmedia/styles.css
# v2026.6.79
...
src/webview/diagram/drawing-feedback.mts
# v2026.6.78
...
src/webview/engines/avr.mtssrc/webview/sim.mts
# v2026.6.70
...
media/styles.csssrc/panel.tsREADME.mdsrc/webview/sim.mts
# v2026.6.69
...
src/webview/engines/i2c-devices.mtssrc/webview/engines/avr.mtssrc/webview/engines/pico.mtssrc/webview/engines/types.mtssrc/webview/diagram/model.mtssrc/webview/diagram/drawing-feedback.mtssrc/webview/composants/externe/lcd.svgsrc/webview/composants/externe/lcd-parallel-20x4.svgsrc/webview/composants/externe/lcd-i2c.svgsrc/webview/composants/externe/lcd-i2c-20x4.svg
# v2026.6.68
...
2. ✅ **Bouton « réinitialiser »** (glyphe `⟲`) agrandi et en gras (`.canvas-controls__btn--reset` : `fonmedia/autoroutage.pngweight: 900`).media/recentrer.svgsrc/panel.ts
...
2. ✅ **Grille seulement sur la feuille de dessin** : nouvemedia/step.pngsheet` (enfant du monde, ancrée à l'origine, s'étend en +x/+y) porte la grille 10 px ; elle suit zoom/translation automatiquement (la grille n'est plus calée à la main dans `applyTransform`). Hors feuille (coords négatives, coin haut-gausrc/panel.tsne `rgb(254,252,238)`** sur `.canvas`. Feuille blanche.
...
2. ✅ **NeoPixel / matrice / anneau colorés depuis le décodage WS2812**. Dessins `neopixel.svg`, `neopixel-matrix.svg` (6src/webview/composants/externe/7seg-2dig.svgregistrés. `reflectNeopixel(svg, colors)` :src/webview/composants/externe/7seg-4dig.svgteinte le diffuseur (plus grand cercle, opacité) ; anneau à `<rect>` = colore le rectangle ; NeoPixel simple = diffuseur du dessin. Couleurs 0..255 du décodeur (`engine.readNeopixel`). Surcharges recalées (probe).
...
5. ℹ️ **À vérifier visuellement par Frank** : alignement des pastilles (surcharges probe) et rendu des écrans/segments/pixels sur chaque de<svg retouche/lcd-i2c.edit.svg>scripts/build-retouche.mjs
...
2. ✅ **Surcharge `resistor` recalée** (nouvelle convention = position absolue viewBox) : broches `1`→`{10,10}`, `2`→`{70,10}` (était `{0,0}`/`{60,0src/webview/composants/externe/resistor.svgastilles tombent pile au bout des pattes grises.src/webview/diagram/board-drawings.mts
...
scripts/build-retouche.mjs<svg retouche/flame.edit.svg>
# v2026.6.59
...
2. ✅ **LED** : `reflectLed` affiche le groupe `<g class="light">` (présent mais `display:none` dans le SVG capté) et teinte le halo src/webview/diagram/drawing-feedback.mtsastel Wokwi). Allumé/éteint suit `ledOn`. Dessin + surcharge enregistrés (`led` déjà calé).
...
2. ✅ **Surcharges recalées** (repère coin haut-gauche feuille, tel quel) dans [`pin-overrides.mts`](src/webview/diagram/pin-overrides.mts) — 8 blocs remplacés (ancienne convention v6.44 → nouvelle) ; `microsd` et `pir` déjà bons (inchangéssrc/webview/elements/boardsus les ronds tombent pile sur leurs headers/psrc/webview/diagram/board-drawings.mts
3. ℹ️ **Sûr car pilotés par l'inspecteur** : ces capteurs prennent leur valeur dans `part.attrs` (disrc/webview/diagram/pin-overrides.mtsnt @wokwi (leur slider n'est pas câblé à la simu) → masquer l'élément ne casse ni la simu ni l'interaction. `npm run verify:all` : 9 suites ✅.
...
2. ✅ **Surcharges de broches uno (31) + nano (36)** extraites des ronds rouges, **repère = coin haut-gascripts/_clean-board-svg.mjsnvention v6.48), dans [`pin-overrides.mts`](src/webview/diagram/pin-overrides.mts). Nsrc/webview/elements/boards/uno.svg3, A4.2/A5.2, ICSP 11.2/12.2/13.2…) src/webview/elements/boards/nano.svgllement (ronds pile sur chaque header, ICSP nano inclsrc/webview/diagram/board-drawings.mts
3. ℹ️ **Outil généralisé** [`scripts/_probe-overrides.mjs`](scripts/_probe-overrides.mjs) (remplace `_probe-mega.mjs`) : extrait les surcharges de n'importe quel `<type>` via src/webview/diagram/pin-overrides.mtsffixes `.OK`/`.ok`/`.PB`. `_clean-board-svg.mjs` gère aussi ces suffixes désormais.
scripts/_probe-overrides.mjs
# v2026.6.53
...
src/webview/programs/mega-pwm345.mjs
# v2026.6.52
...
src/webview/programs/mega-pwm.mjs
# v2026.6.51
...
3. ⏳ **PWM Mega** (point 2 du reste-à-faire 6.50) : non traité. Les broches OC des `MEGA_TIMER*` sont encore celles du 328 (`compPortA/compPinA`…src/webview/programs/mega-demo.mjsvaises broches. À corriger pour le 2560 (OC0A=PB7/pin13, OC0B=PG5/pin4, OC1A=PB5, OC1B=PB6, OC1C=PB7, OC2A=PB4, OC2B=PH6…).
...
2. ✅ **Surcharge `mega` refaite** depuis les ronds, repère = coin haut-gauche feuille (sans marge). Validé : les ronds tombent psrc/webview/elements/boards/mega.svgsrc/webview/diagram/board-drawings.mtssrc/webview/diagram/editor.mtsscripts/_clean-board-svg.mjs
...
2. ✅ **Routage : pas de chevauchement, écart 5 px** ([`autoRoute`](src/webview/diagram/editor.mts)) : les filssrc/webview/diagram/editor.mtsse superposer (coût de **recouvrement colinéaire** d'autres fils) ni se serrer (**pénalité de proximité < 5 px**). Candidats enrichis : coudes en L + détours en Z par la médiane **décalée par voies** (multiples de 5 px) → chaque fil trouve une voie libre ; un tracé droit qui se superposerait reçoit un **créneau** d'évitement. Helpers `collinearOverlap`/`parallelPenalty`/`polylineWireCost`.
3. ✅ **Bases LCD 16×2 et 20×4 (parallèle) à retoucher** : générées src/webview/diagram/editor.mtsvg`](svg%20retouche/lcd.edit.svg) (16×2) et [`lcd-parallel-20x4.edit.svg`](svg%20retouche/lcd-parallel-20x4.edit.svg) (20×4) — variante 16×2 ajoutée à [`scripts/build-retouche.mjs`](scripts/build-retouche.mjs). Anciens `.OK`/`.ok` retirés (à retoucher maintenant).
<svg retouche/lcd.edit.svg><svg retouche/lcd-parallel-20x4.edit.svg>scripts/build-retouche.mjs
# v2026.6.45
...
2. ✅ **uno et 7seg-2dig recalés** (exclus en 6.44) → reportés dans [`pin-overrides.mts`](src/webview/diagram/pin-overrides.mts). uno : 31 brochsrc/webview/diagram/internal-wiring.mtslu par la formule `7seg-${digits}dig` d'`overridesFor`.src/webview/elements/keypad-schema.svgsrc/webview/elements/keypad-3col-schema.svgscripts/_clean-keypad-schema.mjs
src/webview/diagram/pin-overrides.mts
# v2026.6.44
...
2. ✅ **Variantes résolues par `overridesFor`** : 7 seg par `digits` (`7seg-1dig`/`7seg-4dig`), LCD par `pins` (`lcd-i2c` ; parallèle = auto).src/webview/diagram/pin-overrides.mtsscripts/_extract-overrides.mjs
...
1. ✅ Remplace l'icone du bouton avancer d'un pas par media\step.png . Le plus grandscripts/build-retouche.mjs v2026.6.64.
...
2. ✅ **Miniatures fiables** : recalage par `ResizeObserver` au lieu d'une boucle rAF → corrige les vignettes « trop grandes » au lancement et au dépliage d'une section repliée (taille nulle tant que `display:none`).src/webview/diagram/editor.mts
...
2. ✅ **SVG de retouche de tous les composants** dans [`svg retouche/`](svg%20retouche/) (dessin réel + grille 10 px + pastilles nommées `id="pin-<nom>"`) → je relis les positions retouchées vers `pin-overrides`.src/webview/diagram/pin-overrides.mts
3. ✅ **Câblage interne du buzzer supprimé** (plus de bouton K).<svg retouche>
...
2. ✅ **Images des composants** : une image par fiche, **rendue à padocs/composantsposant** (`@wokwi/elements` via Chrome headless, recadrée) ; cartes Uno/Nano/Mega capturées à l'échelle 2× ; Pico/Pico W depuis les SVG maison.
