...
try {
	...
	for (const langue of LANGUES) {
		 ...
		 * 0 = octets (et départ, accusé), 1 = valeurs. Barre de temps 62
		 * (graduations et deux bandes de marqueurs), piste 46, une ligne
		 * d'annotations = 18 (`REGLE_H` et `ANNOT_H` de analyseur-vue.mts).
		 */
		const Y_LIGNE = [0, 1].map((l) => 62 + 46 + 1 + l * 18);
		const releve = async () => {
		...
		// Une piste, deux lignes de décodage : barre 62 + piste 46 + 2 × 18 + 8.
		check('la piste décodée en DHT réserve deux lignes sous sa courbe (canvas de 152 px)',
			Math.round(r.h) === 62 + 46 + 2 * 18 + 8, `hauteur ${r.h}`);
