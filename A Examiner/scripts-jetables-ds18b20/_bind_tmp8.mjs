...
const tmp = mkdtempSync(join(tmpdir(), 'kx-b8-'));
...
for (const [nom, lignes] of Object.entries(CAS)) {
  ...
  d.auto.frontMontant = (c) => {
    const us = desc >= 0 ? (c - desc) / 1000 : -1;
    const imp = fm(c);
    if (us >= 400) evts.push({ t: c / 1e6, us, presence: !!imp });
    return imp;
  };
  let serial = '';
  ...
  console.log(`  RESETS vus (>=400us) : ${evts.length}`);
  for (const e of evts.slice(-12)) console.log(`    t=${e.t.toFixed(1)}ms  bas=${e.us.toFixed(0)}us  presence=${e.presence}`);
}
process.exit(0);
