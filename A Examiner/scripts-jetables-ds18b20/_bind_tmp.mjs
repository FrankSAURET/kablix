...
const tmp = mkdtempSync(join(tmpdir(), 'kx-bind-'));
...
const diag = JSON.parse(readFileSync(process.argv[2], 'utf8'));
for (const cp of diag.customParts ?? []) K.registerCustomPart(cp);
const binds = K.ds18b20Bindings(diag);
console.log('bindings =', JSON.stringify(binds));
...
engine.setDs18b20(binds.map((b) => ({ id: b.partId, pin: b.pin, temperatureC: 23.5 })));
let serial = '';
engine.onSerial = (c) => { serial += c; process.stdout.write(c); };
engine.start();
const t0 = Date.now();
await new Promise((res) => {
  const it = setInterval(() => {
    if (!serial.includes('KX_DONE') && (Date.now() - t0) < 120000) return;
