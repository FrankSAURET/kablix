...
![Barre de simulation](../../media/BarreSimulation.webp)
  ...
  - **Traceur** de courbes
  - **Logic** ouvre l'**analyseur logique** dans un onglet, à poser à côté du schéma. Le bouton n'apparaît qu'une fois une [sonde logique](composants/sonde-logique.md) posée sur une broche — sans pince, l'analyseur n'aurait rien à montrer.
...
- **Pas** : exécute une ligne du fichier source puis se remet en pause. Le panneau **Variables**  montre alors la ligne courante et les variables lisibles du programme ; la ligne est aussi surlignée dans l'éditeur VS Code. Une variable qui vient de changer est affichée en rouge
...
- **Quelles variables sont visibles (C / Arduino)** : celles qui ont une **adresse fixe en mémoire** — les variables globales, et les variables **`static` déclarées dans une fonction**, affichées sous le nom de leur fonction (`loop::memo`). Une variable ordinaire déclarée dans `setup()` ou `loop()` vit dans la pile et n'a pas d'adresse stable : elle n'est pas lisible, mais le panneau la **nomme** pour éviter de la chercher en vain. Deux remèdes : la déclarer hors de toute fonction, ou lui ajouter `static` si sa valeur doit survivre d'un appel au suivant.
...
| ------------------ | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| C / Arduino (Uno)  | Données de débogage extraites à la compilation (`avr-objdump`, fourni avec arduino-cli ou avr-gcc) | variables **globales** et **`static` de fonction** (les locales ordinaires n'ont pas d'adresse fixe ; elles sont nommées, pas lues) ; un `delay()` long avance par tranches de 0,25 s simulée |
