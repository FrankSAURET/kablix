...
> **Lot 4 réalisé** : les points d'arrêt posés dans la gouttière (`vscode.debug.breakpoints`) sont envoyés à la webview (`{ type: 'breakpoints', breakpoints: [{ line, condition? }] }`, 1-based) à chaque changement et après chaque `runProgram`, pour le fichier source courant (.py ou C, pas les artefacts).
...
> **Points d'arrêt conditionnels** : la condition saisie dans la gouttière (`SourceBreakpoint.condition`) est transmise au moteur. En MicroPython, le préambule envoie les arrêts via `\x10{json}\n` (objet `{ "ligne": condition|null }`) et `__kx` n'interrompt sur une ligne conditionnelle que si `eval(condition, globals())` est vrai (erreur d'évaluation = pas d'arrêt, comme VS Code). En C/AVR la condition est ignorée (arrêt inconditionnel — il faudrait un évaluateur d'expression C sur les globales DWARF). Type `Breakpoint` ajouté à `types.mts`. Test : section « breakpoint conditionnel » de `verify-debug-py.mjs`.
