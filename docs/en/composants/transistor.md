...
Transistor in a TO-92 or TO-220 package. In Kablix it acts as a **controlled
switch**: a small base current lets a much larger collector current through, in
the ratio given by the model's **gain**. A MOSFET is driven by **voltage**
instead: its gate is insulated and draws nothing.
...
Those last two never overlap: a MOSFET has no gain, a bipolar no Rds(on). The
picker only shows the one that fits the chosen family.
...
reopens the picker at any time. The **most recently added models** show up in
**blue**.
...
The last entry of the list is always the **custom model** of the chosen family
("Custom NPN", "Custom N-channel MOSFET"…): the criteria you already set are
pre-filled, and **everything stays editable** afterwards — gain or Rds(on), max
voltage and current, package marking, electrode assignment.
...
Pin names **never** change within a family: switching reference therefore
...
and a BD911 is B-C-E, seen from the flat face. The inspector shows that pinout
...
- An NPN conducts when its **base is high and its emitter low**; a PNP, when the
  base is low and the emitter high. An N-channel MOSFET conducts like an NPN:
  gate high, source low.
...
- **Darlington**: two junctions in series, hence **Vbe = 1.4 V** and
  **Vce(sat) = 0.9 V**. Its huge gain (30,000) means a tiny base current is
  enough — that is the whole point.
- **MOSFET**: with an insulated gate there is **no base current and no gain**.
  Voltage alone opens the channel, which then passes up to its **max Id**. A
  gate resistor is therefore not required for it to work (on a real circuit it
  damps the switching edge).
