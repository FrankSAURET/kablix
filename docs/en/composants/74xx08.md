...
DIL-14 package for the breadboard: **four independent AND gates**, each with its two inputs and its output. An AND gate outputs 1 **only when both inputs are 1**.
...
only HC, AC, ACT and C work there — a 74LS08 on a Pico does nothing.
...
|---|---|---|
| `ref` | Part number — changing it changes the pinout, not the drawing | 74xx08 |
...
- AND means “both at once”: a command that only acts when an enable is present.
